        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center text-white">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <div className="flex items-center justify-center gap-3 mb-3">
              ...
              <h1 className="text-4xl md:text-5xl font-bold">On Sale</h1>
              <Tag className="h-6 w-6" />
            </div>
            <p className="text-orange-100 text-lg max-w-2xl mx-auto">
              Every discounted product in one place. Grab them before they are gone.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <motion.div className="mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <div className="flex flex-col gap-4">
            <Tabs value={genderTab} onValueChange={(val) => updateUrl({ category: val === "all" ? undefined : val })} className="w-full">
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3">
              <div className="flex-1">
                <div className="relative">
                  ...
                  <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search sale products..." className="pl-9" />
                </div>
              </div>

              <Select value={derived.sort_by || "newest"} onValueChange={(v) => updateUrl({ sort_by: v || "newest" })}>
        ...
        {/* Products */}
        <ProductGrid
          filters={{ status: "active", is_on_sale: true, sort_by: derived.sort_by || "newest", category: derived.category, search: derived.search }}
          showWishlist
          showAddToCart
        />
      </div>
